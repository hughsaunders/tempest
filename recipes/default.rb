...
if node['openstack']['tempest']['test_img1']['id'].nil?
  ...
  image 'Image setup for cirros-tempest-test' do
    image_url node['openstack']['tempest']['test_img1']['url']
    image_name 'cirros-tempest-test'
    identity_user node['openstack']['tempest']['user1']
    identity_pass node['openstack']['tempest']['user1_pass']
    identity_tenant node['openstack']['tempest']['user1_tenant']
    keystone_uri identity_admin_endpoint['uri']
    action :upload
  end
else
...
if node['openstack']['tempest']['test_img1']['id'].nil?
  ruby_block 'get_image1_uuid' do
    action :create
    block do
      shell_cmd = "nova --no-cache --os-username=#{node['openstack']['tempest']['user1']} --os-password=#{node['openstack']['tempest']['user1_pass']} --os-tenant-name=#{node['openstack']['tempest']['user1_tenant']} --os-auth-url=#{identity_admin_endpoint['uri']} image-show cirros-#{node['openstack']['tempest']['user1_tenant']}-image | awk '{if($2==\'id\') print $4}'"
      ...
      if img1_uuid.length > 0
        ...
        node.set['openstack']['tempest']['tempest_img1_uuid'] = img1_uuid
      else
        node.set['openstack']['tempest']['tempest_img1_uuid'] = 'Failed to get uploaded image id'
      end
    end
  end
else
  node.set['openstack']['tempest']['tempest_img1_uuid'] = node['openstack']['tempest']['test_img1']['id']
end

unless node['keystone']['users'][node['keystone']['admin_user']]['password'].nil?
...
template '/opt/tempest/etc/tempest.conf' do
  ...
  variables(
            ...
            'identity_endpoint_host' => ks_service_endpoint['host'],
            'identity_endpoint_port' => ks_service_endpoint['port'],
            'tempest_tenant_isolation' => node['openstack']['tempest']['tenant_isolation'],
            'tempest_tenant_reuse' => node['openstack']['tempest']['tenant_reuse'],
            'tempest_user1' => node['openstack']['tempest']['user1'],
            'tempest_user1_pass' => node['openstack']['tempest']['user1_pass'],
            'tempest_user1_tenant' => node['openstack']['tempest']['user1_tenant'],
            'tempest_img_flavor1' => node['openstack']['tempest']['img1_flavor'],
            'tempest_img_flavor2' => node['openstack']['tempest']['img2_flavor'],
            ...
            'tempest_user2' => node['openstack']['tempest']['user2'],
            'tempest_user2_pass' => node['openstack']['tempest']['user2_pass'],
            'tempest_user2_tenant' => node['openstack']['tempest']['user2_tenant'],
            'tempest_img_ref1' => node['openstack']['tempest']['tempest_img1_uuid'],
            'tempest_img_ref2' => node['openstack']['tempest']['tempest_img1_uuid']
            )
end
